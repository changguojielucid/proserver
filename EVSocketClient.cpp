...
EVSocketClient::EVSocketClient(QObject *parent) :
  QObject(parent)
{
  QThreadPool::globalInstance()->setMaxThreadCount(5);
}

void EVSocketClient::setSocket(qintptr descriptor)
{
  // make a new socket
...
void EVSocketClient::connected()
{
  qDebug() << "Client connected event";
}

// asynchronous
void EVSocketClient::disconnected()
{
...
void EVSocketClient::readyRead()
{
  qDebug() << "MyClient::readyRead()";
  qDebug() << socket->readAll();

  // Time consumer
  EVTask *mytask = new EVTask();
...
void EVSocketClient::TaskResult(int Number)
{
  QByteArray Buffer;
  Buffer.append("\r\nTask result = ");
